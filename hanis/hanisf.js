...
var HAniSF = function() {
  ...
  stepDwell, lastDwell, delay, enableSmoothing, smoothingQuality, overlaySmoothing,
  ...
  doDistance, doDirection, x0Dist, y0Dist, distDigits, distShift, hasCoords,
              ...
              for (j=0; j<numOverlays; j++) {
                ...
                if (useForAll[j+1] == undefined || useForAll[j+1] == k) {
                  ...
                  overlayImages[k][j].onload = function() {
                    ...
                    imgGotCount++;
                    drawImageProgress();
                    if (chkImageSize && (this.height != imgHChk || this.width != imgWChk)) {
                      this.gotit = false;
  ...
  function drawLines() {
    ...
    if (showDistance) {
      ...
      var distVal = "";
      if (!hasCoords) {
        var dx = distXScale*(loc0ll[0] - locll[0]);
        var dy = distYScale*(loc0ll[1] - locll[1]);
